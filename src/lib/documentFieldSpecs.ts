...
export interface DocumentSubtypeSpec {
  key: string
  label: string
}
...
const spec = (key: string, label: string): DocumentFieldSpec => ({ key, label })
...
function fields(keys: string[]) {
  return keys.map((key) => spec(key, fieldLabelByKey[key] ?? key))
}
...
export function documentFieldLabel(key: string) {
...
export function expectedFieldKeysForExtracted(kind: DocumentKind, extracted: Record<string, unknown> = {}) {
