...
function mergeDocument(profile: FinancialProfile, document: ImportedDocument) {
  ...
  const hasManualApproval = Boolean(existing.extracted?.reviewedMovementRowsAppliedAt || existing.extracted?.reviewedPositionRowsAppliedAt)
...
export function applyReviewedStatementMovements(profile: FinancialProfile, documentId: string): ApplyReviewedMovementsResult {
  ...
  if (!isInvestmentStatement && document.kind !== 'bank_statement' && document.kind !== 'credit_card_statement') {
    throw new Error('Solo se pueden aplicar movimientos revisados de estados bancarios, tarjetas o posiciones de inversion.')
  }
  ...
  const accountType = isInvestmentStatement ? (documentExtractedString(document, 'accountType') === 'retirement' ? 'retirement' : 'investment') : isCreditCardStatement ? 'credit_card' : reviewedStatementAccountType(document)
  if (!rows.length && !isInvestmentStatement) throw new Error('El documento no tiene movimientos visibles para aplicar.')
